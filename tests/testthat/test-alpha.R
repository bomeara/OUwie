...
test_that("testing likelihood matches", {
	...
	set.seed(1137)
	tree <- phytools::make.simmap(tree, disc_trait)
...
test_that("testing vcv", {
	...
	root.state <- which(
		colnames(tree$mapped.edge) ==
			names(tree$maps[[root.edge.index[2]]][1])
	)
	
	ouwie_vcv <- OUwie:::varcov.ou.enhanced.tree(
		phy = tree,
		enhanced_tree = create_enhanced_tree_structure(tree),
