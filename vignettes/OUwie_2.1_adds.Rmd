...
```{r, eval=TRUE}
data(tworegime)
pp <- OUwie(tree, trait, model="OUM", root.station=TRUE, scaleHeight=FALSE, shift.point=0, algorithm="invert", quiet=TRUE)
pp
```
...
```{r, eval=TRUE}
data(tworegime)
pp <- OUwie(tree, trait, model="OUM", root.station=FALSE, scaleHeight=FALSE, shift.point=0, algorithm="invert", quiet=TRUE)
pp
```
...
data(tworegime)
set.seed(42)
ouwiefit <- OUwie(tree, trait, model="OUM", scaleHeight=FALSE, root.station=FALSE, shift.point=0.5, algorithm="invert", quiet=TRUE, check.identify=FALSE)
```
...
```{r, eval=FALSE, echo=FALSE, fig.height=10, fig.width=6.5, fig.cap = "A plot of the ancestral state reconstruction under an OUwie model."}
...
```{r, eval=TRUE, echo=TRUE}
data(tworegime)
three.point <- OUwie(tree, trait, model="OUMV", root.station=FALSE, scaleHeight=FALSE, shift.point=0.5, algorithm="three.point", quiet=TRUE, check.identify=FALSE)
three.point
```
...
```{r, eval=TRUE, echo=TRUE}
data(tworegime)
invert <- OUwie(tree, trait, model="OUMV", root.station=FALSE, scaleHeight=FALSE, shift.point=0.5, algorithm="invert", quiet=TRUE)
invert
```
...
```{r, eval=FALSE, echo=TRUE}
data(tworegime)
invert <- OUwie(tree, trait, model="OUMV", root.station=FALSE, scaleHeight=FALSE, shift.point=0.5, tip.fog="estimate", algorithm="invert", quiet=TRUE)
invert
```
